...
export const postTransfer: any = createAsyncThunk(
    `user/postTransfer`, async ({body, token}: {body: pfTransfer | bankTransfer, token: string}, { dispatch, rejectWithValue }) => {

        try {
            const { data }: any = await axios.post(`https://api.payfocuss.com/transfer`, body, {
                headers: {
...
export const lookUp: any = createAsyncThunk(
    `user/lookUp`, async ({email, token}: {email: string, token: string}, { dispatch, rejectWithValue }) => {

        try {
            const { data }: any = await axios.get(`https://api.payfocuss.com/lookup/${email}`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                }
